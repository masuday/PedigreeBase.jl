...
Computes the inverse of numerator (additive) relationship matrix using the "Henderson's" method from the pedigree list `pedlist` and inbreeding coefficients `f`. See Henderson (1976) and Quaas (1976) for details.
...
function get_nrminv(pedlist::Matrix{Ti}; f::Vector{Tv}=zeros(Tv,0), rank=0) where {Ti<:Integer,Tv<:AbstractFloat}
   if length(f)==0
      inb = zeros(Tv)
   else
      inb = f
